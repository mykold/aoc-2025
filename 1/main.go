...
// This has given them the tools they need to prevent their usual
// Christmas emergency. For example, they now know that the North
// Pole decorations need to be finished soon so that other critical
...
//
// To save Christmas, the Elves need you to finish decorating the
// North Pole by December 12th.
//
// Collect stars by solving puzzles. Two puzzles will be made
// available on each day; the second puzzle is unlocked when you
// complete the first. Each puzzle grants one star. Good luck!
//
// You arrive at the secret entrance to the North Pole base ready
// to start decorating. Unfortunately, the password seems to have
// been changed, so you can't get in. A document taped to the wall
// helpfully explains:
//
...
//
// The safe has a dial with only an arrow on it; around the dial are
// the numbers 0 through 99 in order. As you turn the dial, it make
// a small click noise as it reaches each number.
//
// The attached document (your puzzle input) contains a sequence
// rotations, one per line, which tell you how to open the safe.
// A rotation starts with an L or R which indicates whether the
// rotation should be to the left (toward lower numbers) or to
// the right (toward higher numbers). Then, the rotation has a
// distance value which indicates how many clicks the dial should
// be rotated in that direction.
//
...
// one click makes it point at 99. Similarly, turning the dial
...
//
// For example, suppose the attached document contained the
// following rotations:
//
// L68
// L30
...
//
// As you're rolling the snowballs for your snowman, you find
// another security document that must have fallen into the snow:
//
...
//
// Following the same rotations as in the above example, the dial
// points at zero a few extra times during its rotations:
//
// The dial starts by pointing at 50.
...
//
// Be careful: if the dial were pointing at 50, a single rotation lik
...
func ParseRotation(r string) (string, int) {
	i, err := strconv.ParseInt(r[1:], 10, 16)
	if err != nil {
		panic(err)
	}
	return r[:1], int(i)
...
func main() {
	...
	for scanner.Scan() {
		typ, shift := ParseRotation(scanner.Text())
